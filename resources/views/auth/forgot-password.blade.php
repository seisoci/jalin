        ...
        <div class="card card-transparent auth-card shadow-none d-flex justify-content-center mb-0">
          <div class="card-body">
            <a href="#" class="navbar-brand d-flex align-items-center gap-2 mb-3">
              ...
              <h6 class="logo-title d-none d-sm-block mb-0">{{env('APP_NAME')}}</h6>
            </a>
            <h2 class="mb-2">Reset Password</h2>
            <p>Masukkan alamat email Anda dan kami akan mengirimkan email berisi petunjuk untuk mengatur ulang kata
              sandi Anda.</p>
            <!-- Session Status -->
            @if(!empty(session('status')) && $errors->isEmpty())
            ...
            <form method="POST" action="{{ route('sentresetpassword') }}">
              @csrf
              <div class="row">
                <div class="col-lg-12">
                  <div class="floating-label form-group">
                    ...
                    <input type="email" name="email" :value="old('email')" class="form-control" id="email"
                           aria-describedby="email" placeholder="demo@example.com" required autofocus>
                  </div>
                </div>
              </div>
